...
use nocheat::{build_dataframe, df_to_ndarray, train_model, generate_default_model};
...
fn bench_train_model(c: &mut Criterion) {
    ...
    c.bench_function("train_model_100", |b| b.iter(|| {
        let _ = train_model(
...
fn bench_generate_default_model(c: &mut Criterion) {
    ...
    c.bench_function("generate_default_model", |b| b.iter(|| {
