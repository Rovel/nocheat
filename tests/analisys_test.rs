use nocheat::{build_dataframe, df_to_ndarray, train_model, generate_default_model, analyze_stats};
...
fn test_training_workflow() {
    ...
    println!("Checking if cheat_model.bin exists: {}", std::path::Path::new("cheat_model.bin").exists());
    let file_size = std::fs::metadata("cheat_model.bin").map(|m| m.len()).unwrap_or(0);
    ...
    assert!(normal_score < suspicious_score, 
...
fn test_generate_default_model() {
    ...
    assert!(score > 0.7, 
        "Expected suspicious player to have a high score with default model, got {}", score);
